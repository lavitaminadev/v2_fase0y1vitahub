...
function previewUrl(url?: string): string | undefined {
  ...
  return safe.includes('drive.google.com') ? safe.replace(/\/view.*$/, '/preview') : safe;
}
...
export function ApprovalsPage() {
  ...
  return <div className="page approvals-workspace">
    <div className="page-header"><div><span className="page-eyebrow">DECISIONES CON EVIDENCIA</span><h1>Aprobaciones visuales</h1><p className="page-subtitle">Compara, revisa y decide.</p></div><div className="approval-tabs"><button className={tab === 'pending' ? 'active' : ''} onClick={() => setTab('pending')}>Pendientes <span>{pending.length}</span></button><button className={tab === 'history' ? 'active' : ''} onClick={() => setTab('history')}>Historial <span>{resolved.length}</span></button></div></div>
