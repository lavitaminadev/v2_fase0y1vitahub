...
export function ReservationsPage({ clientView = false }: { clientView?: boolean }) {
  ...
  const couponCreate = useMutation({
    ...
    onSuccess: () => { setCouponForm({ code: '', discountType: 'percentage', value: 0, maxUses: 0, validFrom: '', validUntil: '', formIds: '', validDaysOfWeek: [] }); setCouponTab('list'); qc.invalidateQueries({ queryKey: ['coupons'] }); },
  });
