...
export function ReservationsPage({ clientView = false }: { clientView?: boolean }) {
  ...
  return <div className="page reservation-module">
    <section className="reservation-page-head">
      <div><span className="reservation-brand">RESERVAS</span><h1>Operación de reservas</h1><p>Agenda, solicitudes, asistencia y resultados por cliente.</p></div>
      {!clientView && <button className="btn reservation-cta" onClick={() => { setFormData((current) => ({ ...current, clientId: clientFilter })); setCreateOpen(true); }}>Nueva agenda</button>}
    </section>
    <nav className="reservation-tabs" aria-label="Secciones de reservas">
      {(clientView
        ? ([['forms', 'Agenda'], ['bookings', 'Reservas'], ['metrics', 'Resultados']] as const)
        : ([['forms', 'Agendas'], ['bookings', 'Reservas'], ['metrics', 'Resultados'], ['coupons', 'Cupones']] as const)
    ...
    {tab === 'forms' && <section>
      <div className="reservation-section-head"><div><span className="page-eyebrow">AGENDA</span><h2>Canales de reserva</h2></div><p>{visibleForms.length} de {clientForms.length} visibles</p></div>
      ...
      <div className="reservation-form-filters"><input className="input" type="search" aria-label="Buscar agenda" placeholder="Buscar agenda" value={formFilters.search} onChange={(event) => setFormFilters((current) => ({ ...current, search: event.target.value }))} />{!clientView && <select className="input" aria-label="Filtrar agendas por cliente" value={clientFilter} onChange={(event) => setClientFilter(event.target.value)}><option value="">Todos los clientes</option>{clients.map((client) => <option value={client.id} key={client.id}>{client.name}</option>)}</select>}<select className="input" aria-label="Filtrar agendas por estado" value={formFilters.status} onChange={(event) => setFormFilters((current) => ({ ...current, status: event.target.value }))}><option value="">Todos los estados</option><option value="published">Publicados</option><option value="paused">Pausados</option><option value="draft">Borradores</option></select></div>
      {visibleForms.length === 0 ? <div className="reservation-empty"><strong>Sin agendas configuradas</strong><p>Crea un canal de reserva para comenzar a recibir solicitudes.</p>{!clientView && <button className="btn btn-primary" onClick={() => { setFormData((current) => ({ ...current, clientId: clientFilter })); setCreateOpen(true); }}>Crear agenda</button>}</div> : <div className="reservation-form-grid">
        {visibleForms.map((form) => <article className="reservation-form-card" key={form.id}>
          ...
          <h3>{form.name}</h3><p>Canal público configurado</p>{!clientView && <small className="form-client-name">{clients.find((client) => client.id === form.clientId)?.name || 'Cliente no disponible'}</small>}
          ...
          <div className="form-card-actions"><Link className="btn btn-primary btn-sm" to={formPath(form.id)}>{clientView ? 'Configurar' : 'Administrar'}</Link>{safeUrl(formPublicUrl(form)) ? <a className="btn btn-outline btn-sm" href={safeUrl(formPublicUrl(form))} target="_blank" rel="noreferrer">Vista pública</a> : null}{!clientView && <button className="btn btn-outline btn-sm" disabled={duplicateMutation.isPending} onClick={() => setConfirmFormAction({ id: form.id, action: 'duplicate' })}>{duplicateMutation.isPending ? 'Duplicando...' : 'Duplicar'}</button>}<button className="btn btn-outline btn-sm" onClick={() => backupForm(form)}>Respaldar</button>{!clientView && form.status !== 'draft' && <button className="btn btn-outline btn-sm" disabled={updateFormMutation.isPending} onClick={() => form.status === 'paused' ? updateFormMutation.mutate({ id: form.id, status: 'published' }) : setConfirmFormAction({ id: form.id, action: 'pause' })}>{updateFormMutation.isPending ? 'Procesando...' : form.status === 'paused' ? 'Reanudar' : 'Pausar'}</button>}</div>
        </article>)}
      </div>}
    </section>}
