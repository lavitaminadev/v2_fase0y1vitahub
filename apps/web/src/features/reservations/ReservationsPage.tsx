...
export function ReservationsPage({ clientView = false }: { clientView?: boolean }) {
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (blob) => { setExportError(''); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `reservas-${new Date().toISOString().slice(0, 10)}.csv`; anchor.click(); URL.revokeObjectURL(url); },
  ...
  const couponCreate = useMutation({
    ...
    onSuccess: () => { setCouponForm({ code: '', discountType: 'percentage', value: 0, maxUses: 0, validFrom: '', validUntil: '', formIds: '', validDaysOfWeek: [] }); setCouponCreateOpen(false); qc.invalidateQueries({ queryKey: ['coupons'] }); },
  });
  ...
  return <div className="page reservation-module">
    ...
    {tab === 'bookings' && <section>
      <div className="reservation-section-head"><div><span className="page-eyebrow">OPERACIÓN DIARIA</span><h2>Lista de reservas</h2></div><div className="reservation-actions"><button className="btn btn-outline btn-sm" onClick={() => setManualOpen(true)}>Agregar reserva manual</button><button className="btn btn-outline btn-sm" onClick={() => exportMutation.mutate()} disabled={exportMutation.isPending}>{exportMutation.isPending ? 'Preparando...' : 'Exportar CSV'}</button></div></div>{exportError && <div className="alert alert-error">{exportError}</div>}
