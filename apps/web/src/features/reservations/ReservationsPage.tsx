...
export function ReservationsPage({ clientView = false }: { clientView?: boolean }) {
  ...
  const [tab, setTab] = useState<'forms' | 'bookings' | 'metrics'>(requestedTab === 'bookings' || requestedTab === 'metrics' ? requestedTab : 'forms');
  ...
  return <div className="page reservation-module">
    ...
    <nav className="reservation-tabs" aria-label="Secciones de reservas">
      {([['forms', 'Formularios y encuestas'], ['bookings', 'Datos recopilados'], ['metrics', 'Analítica de reservas']] as const).map(([key, label]) => <button key={key} className={tab === key ? 'active' : ''} onClick={() => setTab(key)}>{label}{key === 'bookings' && bookingPage?.total ? <span>{bookingPage.total}</span> : null}</button>)}
    </nav>
