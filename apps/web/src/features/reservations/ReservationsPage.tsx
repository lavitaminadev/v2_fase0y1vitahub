...
export function ReservationsPage({ clientView = false }: { clientView?: boolean }) {
  ...
  return <div className="page reservation-module">
    ...
    {tab === 'bookings' && <section>
      ...
      {loadingBookings && !bookingPage ? <LoadingSpinner text="Buscando reservas..." /> : bookings.length === 0 ? <div className="reservation-empty"><strong>Sin reservas para estos filtros</strong><p>Las nuevas solicitudes aparecerán aquí en tiempo real.</p></div> : <div className="booking-list">
        {bookings.map((item) => { const next = NEXT_STATUSES[item.status] || []; return <article className="booking-row" key={item.id}>
          ...
          <select className="input booking-status" aria-label={`Cambiar estado de ${item.guestName}`} value={updateMutation.isPending ? item.status : item.status} disabled={next.length === 0 || updateMutation.isPending} onChange={(event) => updateMutation.mutate({ id: item.id, body: { status: event.target.value } })} style={updateMutation.isPending ? { opacity: 0.6 } : undefined}><option value={item.status}>{STATUS_LABELS[item.status] || item.status}</option>{next.map((status) => <option value={status} key={status}>{STATUS_LABELS[status]}</option>)}</select>
        </article>; })}
      </div>}
    ...
    {tab === 'metrics' && <section>
      <div className="reservation-section-head"><div><span className="page-eyebrow">ANALYTICS</span><h2>Embudo, demanda y asistencia</h2></div><div className="days-selector">{[7, 14, 30, 60, 90, 180, 365].map((d) => <button key={d} className={`btn btn-${metricsDays === d ? 'primary' : 'outline'} btn-xs`} onClick={() => setMetricsDays(d)}>{d}d</button>)}</div></div>
      <div className="reservation-funnel"><div><span>Visitas</span><strong>{metrics?.funnel.views || 0}</strong></div><i /><div><span>Inicios</span><strong>{metrics?.funnel.starts || 0}</strong></div><i /><div><span>Reservas</span><strong>{metrics?.funnel.completed || 0}</strong></div><div className="funnel-rate"><span>Conversión</span><strong>{metrics?.funnel.conversionRate != null ? `${metrics.funnel.conversionRate}%` : '—'}</strong></div></div>
      ...
      {!clientView && clientFilter && <div className="client-dashboard"><div className="reservation-section-head"><div><span className="page-eyebrow">DASHBOARD {clients.find((c) => c.id === clientFilter)?.name?.toUpperCase()}</span><h3>Formularios activos</h3></div></div><div className="reservation-form-mini-grid">{forms.filter((f) => f.clientId === clientFilter).map((form) => <div className="form-mini-card" key={form.id}><h4>{form.name}</h4><div className="form-mini-stats"><span>{form.status === 'published' ? 'Publicado' : form.status === 'paused' ? 'Pausado' : 'Borrador'}</span><span>{form.durationMinutes} min</span><span>{form.fieldSchema.length} campos</span></div><Link className="btn btn-outline btn-xs" to={formPath(form.id)}>Abrir</Link></div>)}</div></div>}
      <div className="reservation-operations-map"><div className="reservation-section-head"><div><span className="page-eyebrow">MAPA DE OCUPACIÓN</span><h3>Calendario operacional, últimos {metricsDays} días</h3></div><p>Intensidad según reservas reales</p></div><div className="reservation-heatmap">{Array.from({ length: metricsDays }, (_, index) => { const date = new Date(); date.setHours(12, 0, 0, 0); date.setDate(date.getDate() - (metricsDays - 1) + index); const key = date.toISOString().slice(0, 10); const count = dailyTotals[key] || 0; return <div key={key} title={`${date.toLocaleDateString('es-CL')}: ${count} reserva(s)`} style={{ '--heat': String(count / maxDailyTotal) } as React.CSSProperties}><span>{date.getDate()}</span><small>{date.toLocaleDateString('es-CL', { weekday: 'short' }).slice(0, 2)}</small><strong>{count}</strong></div>; })}</div><footer><span>Baja demanda</span><i /><i /><i /><i /><span>Alta demanda</span></footer></div>
      <div className="reservation-analytics"><div><h3>Origen y campaña</h3>{metrics?.sources.length ? metrics.sources.map((source) => <div className="source-row" key={`${source.source}-${source.campaign}`}><div><strong>{source.campaign}</strong><small>{source.source}</small></div><span>{source.total} reservas</span><em>{source.attended} asistencias</em></div>) : <EmptyState icon="📡" title="Sin atribución" description="Aún no hay campañas o fuentes registradas para este período." />}</div><div><h3>Horas más demandadas</h3><div className="hour-chart">{Array.from({ length: 12 }, (_, index) => index + 8).map((hour) => { const count = (metrics?.daily || []).filter((entry) => Number(entry.hour) === hour).reduce((sum, entry) => sum + Number(entry.total), 0); const max = Math.max(...(metrics?.daily || []).map((entry) => Number(entry.total)), 1); return <div key={hour} title={`${count} reserva(s) a las ${hour}:00`}><i style={{ height: `${Math.max(count / max * 100, 3)}%` }} /><span>{hour}:00</span></div>; })}</div></div></div>
    </section>}
