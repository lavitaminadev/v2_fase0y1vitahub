...
export function ReservationBuilderPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (next) => { setDraft(next); setSaved(true); qc.invalidateQueries({ queryKey: ['reservation-forms'] }); triggerToast('Formulario guardado'); },
  });
  ...
  return <div className="reservation-builder">
    <header className="builder-top">
      <div><Link to={clientMode ? '/portal/reservations' : '/reservations'}>← Reservas</Link><div><input type="text" autoComplete="off" aria-label="Nombre del formulario" value={draft.name} disabled={clientMode} onChange={(event) => change({ name: event.target.value })} /><span className={saveMutation.isPending ? 'saving' : saved ? 'saved' : 'unsaved'}>{saveMutation.isPending ? 'Guardando...' : saved ? 'Todos los cambios guardados' : 'Cambios sin guardar'}</span></div></div>
