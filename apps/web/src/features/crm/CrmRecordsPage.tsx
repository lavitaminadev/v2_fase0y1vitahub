...
const DEAL_STAGE_COPY: Record<string, { label: string; intent: string; defaultProbability: number }> = {
  new: { label: 'Prospecto', intent: 'Cuenta detectada, falta validar interes', defaultProbability: 10 },
  qualified: { label: 'Diagnostico', intent: 'Dolor confirmado y reunion activa', defaultProbability: 35 },
  proposal: { label: 'Propuesta', intent: 'Oferta enviada, esperando respuesta', defaultProbability: 55 },
  negotiation: { label: 'Negociacion', intent: 'Precio, alcance o fecha en cierre', defaultProbability: 75 },
  won: { label: 'Ganado', intent: 'Pasa a onboarding y operacion', defaultProbability: 100 },
  lost: { label: 'Perdido', intent: 'Cerrar motivo y aprendizaje', defaultProbability: 0 },
};
...
export function ContactsPage() {
  ...
  const activeOperationalContacts = statusCounts.new + statusCounts.reserved;
  ...
  <CrmScopeBanner
    mode="operations"
    title="Comensales y contactos por cliente"
    description="Este bloque corresponde al CRM operativo del restaurante: personas que reservaron, asistieron o no asistieron."
    clientName={activeClientName}
    globalLabel="Vista global de operacion por clientes"
  />

  <section className="reservation-ops-strip" aria-label="Resumen operativo de comensales">
    <article>
      <span>Contactos activos</span>
      <strong>{activeOperationalContacts}</strong>
      <small>Nuevos o con reserva pendiente.</small>
    </article>
    <article>
      <span>Asistencia</span>
      <strong>{attendanceRate === null ? '-' : `${attendanceRate}%`}</strong>
      <small>Asistio sobre asistio + no asistio.</small>
    </article>
    <article>
      <span>Vista actual</span>
      <strong>{activeClientName ?? 'Global'}</strong>
      <small>{user?.role === 'client' ? 'Solo tu marca' : 'Filtrable por cliente'}</small>
    </article>
  </section>
...
export function OpportunitiesPage() {
  ...
  const opportunitiesQuery = useQuery<PageResult<Opportunity>>({ queryKey: ['crm-opportunities', clientFilter], queryFn: () => api.get(`/crm/opportunities?limit=100${clientFilter ? `&clientId=${encodeURIComponent(clientFilter)}` : ''}`) });
  const leadsQuery = useQuery<{ data: LeadOption[] }>({ queryKey: ['leads', clientFilter], queryFn: () => api.get(`/crm/leads${clientFilter ? `?clientId=${encodeURIComponent(clientFilter)}` : ''}`) });
  ...
  const activeClientName = clientFilter ? (oppClients.find((client) => client.id === clientFilter)?.name ?? 'Cliente no encontrado') : undefined;
  ...
  <CrmScopeBanner
    mode="commercial"
    title="Pipeline comercial y forecast"
    description="Negocios de La Vitamina con restaurantes o cuentas prospecto. No corresponde al CRM operativo de comensales."
    clientName={activeClientName}
    globalLabel="Vista global de negocios de agencia"
  />
  <WorkspaceHeader eyebrow="VENTAS LA VITAMINA" title="Pipeline de negocios" description="Cada oportunidad representa una cuenta comercial: monto, probabilidad, cierre esperado y proxima accion obligatoria." action={() => openEdit()} />
  ...
  <div className="filters opportunity-filters"><input className="input" type="search" placeholder="Buscar oportunidad, lead o cliente" value={search} onChange={(event) => setSearch(event.target.value)} /><select className="input" value={clientFilter} onChange={(event) => setClientFilter(event.target.value)}><option value="">Todos los clientes</option>{oppClients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select><select className="input" value={stageFilter} onChange={(event) => setStageFilter(event.target.value)}><option value="">Todas las etapas</option>{STAGES.map((stage) => <option key={stage} value={stage}>{statusLabel(stage)}</option>)}</select><div className="view-toggle" aria-label="Vista de oportunidades"><button className={view === 'board' ? 'active' : ''} onClick={() => setView('board')}>Tablero</button><button className={view === 'table' ? 'active' : ''} onClick={() => setView('table')}>Lista</button></div><span className="filter-result-count">{opportunities.length} resultados</span></div>
  {view === 'board' ? <div className="opportunity-board commercial-deal-board">{STAGES.filter((stage) => !stageFilter || stage === stageFilter).map((stage) => { const stageItems = opportunities.filter((item) => item.stage === stage); const stageAmount = stageItems.reduce((sum, item) => sum + Number(item.amount || 0), 0); return <section key={stage} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const id = event.dataTransfer.getData('opportunity-id'); if (id) moveStage.mutate({ id, stage }); }}><header><div><strong>{DEAL_STAGE_COPY[stage]?.label ?? statusLabel(stage)}</strong><span>{stageItems.length}</span></div><small>{DEAL_STAGE_COPY[stage]?.intent}</small><em>CLP {Math.round(stageAmount).toLocaleString('es-CL')}</em></header><div>{stageItems.map((item) => { const overdueCard = item.nextActionAt && new Date(item.nextActionAt) < new Date() && !['won', 'lost'].includes(item.stage); const accountName = oppClients.find((client) => client.id === item.clientId)?.name || oppLeads.find((lead) => lead.id === item.leadId)?.name || 'Sin cuenta asociada'; return <article draggable onDragStart={(event) => event.dataTransfer.setData('opportunity-id', item.id)} key={item.id} className={overdueCard ? 'is-overdue' : ''}><button className="opportunity-card-main" onClick={() => openEdit(item)}><strong>{item.name}</strong><small>{accountName}</small></button><div className="opportunity-value"><strong>CLP {Number(item.amount || 0).toLocaleString('es-CL')}</strong><span>{item.probability}%</span></div><div className="deal-health-row"><span>{ageInDays(item.createdAt)} dias abierto</span><span>{item.expectedCloseDate ? `Cierre ${new Date(item.expectedCloseDate).toLocaleDateString('es-CL')}` : 'Sin cierre'}</span></div><div className={`deal-next-action ${overdueCard ? 'is-overdue' : ''}`}><span>{overdueCard ? 'Accion vencida' : 'Proxima accion'}</span><strong>{item.nextAction || 'Definir seguimiento'}</strong><small>{item.nextActionAt ? new Date(item.nextActionAt).toLocaleString('es-CL', { dateStyle: 'short', timeStyle: 'short' }) : 'Sin fecha asignada'}</small></div></article>; })}</div>{stageItems.length === 0 && <p>Suelta una oportunidad aqui</p>}</section>; })}</div> : <DataTable<Opportunity> keyExtractor={(item) => item.id} data={opportunities} emptyMessage="Aún no hay oportunidades" columns={[
  ...
  <Modal open={open} onClose={() => setOpen(false)} title={editing ? 'Actualizar oportunidad' : 'Nueva oportunidad'}><form className="modal-form" onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>{save.error && <div className="alert alert-error">{save.error.message}</div>}<label>Nombre del negocio<input className="input" required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Ej. Plan reservas + Meta para Restaurante Norte" /></label><div className="form-row"><label>Valor estimado<input className="input" type="number" min="0" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} /></label><label>Fecha de cierre<input className="input" type="date" value={form.expectedCloseDate} onChange={(event) => setForm({ ...form, expectedCloseDate: event.target.value })} /></label></div><div className="form-row"><label>Etapa<select className="input" value={form.stage} onChange={(event) => { const stage = event.target.value; setForm({ ...form, stage, probability: String(DEAL_STAGE_COPY[stage]?.defaultProbability ?? form.probability) }); }}>{STAGES.map((stage) => <option key={stage} value={stage}>{DEAL_STAGE_COPY[stage]?.label ?? statusLabel(stage)}</option>)}</select></label><label>Probabilidad<input className="input" type="number" min="0" max="100" value={form.probability} onChange={(event) => setForm({ ...form, probability: event.target.value })} /></label></div><div className="form-row"><label>Próxima acción obligatoria<input className="input" required value={form.nextAction} onChange={(event) => setForm({ ...form, nextAction: event.target.value })} placeholder="Ej. Enviar propuesta corregida" /></label><label>Fecha y hora de seguimiento<input className="input" type="datetime-local" value={form.nextActionAt} onChange={(event) => setForm({ ...form, nextActionAt: event.target.value })} /></label></div><div className="form-row"><label>Prospecto origen<select className="input" value={form.leadId} onChange={(event) => setForm({ ...form, leadId: event.target.value })}><option value="">Sin prospecto</option>{oppLeads.map((lead) => <option key={lead.id} value={lead.id}>{lead.name}</option>)}</select></label><label>Cuenta / cliente agencia<select className="input" value={form.clientId} onChange={(event) => setForm({ ...form, clientId: event.target.value })}><option value="">Sin cuenta</option>{oppClients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></label></div><div className="modal-actions"><button type="button" className="btn btn-outline" onClick={() => setOpen(false)}>Cancelar</button><button className="btn btn-primary" disabled={save.isPending}>{save.isPending ? 'Guardando...' : 'Guardar oportunidad'}</button></div></form></Modal>
...
export function InteractionsPage() {
  ...
  const [feedback, setFeedback] = useState('');
  const interactionsQuery = useQuery<PageResult<Interaction>>({ queryKey: ['crm-interactions', clientFilter], queryFn: () => api.get(`/crm/interactions?limit=100${clientFilter ? `&clientId=${encodeURIComponent(clientFilter)}` : ''}`) });
  ...
  const opportunitiesQuery = useQuery<PageResult<Opportunity>>({ queryKey: ['crm-opportunities', clientFilter], queryFn: () => api.get(`/crm/opportunities?limit=100${clientFilter ? `&clientId=${encodeURIComponent(clientFilter)}` : ''}`) });
  const clientsQuery = useQuery<{ data: ClientOption[] }>({ queryKey: ['clients'], queryFn: () => api.get('/clients') });
  const activeClientName = clientFilter ? ((clientsQuery.data?.data ?? []).find((client) => client.id === clientFilter)?.name ?? 'Cliente no encontrado') : undefined;
  ...
  <CrmScopeBanner
    mode="commercial"
    title="Actividad comercial de La Vitamina"
    description="Seguimientos, reuniones y correos asociados a prospectos y cuentas comerciales de la agencia."
    clientName={activeClientName}
    globalLabel="Vista global de actividad comercial"
  />
  <WorkspaceHeader eyebrow="BANDEJA DIARIA" title="Centro de actividad comercial" description="Prioriza seguimientos, reuniones y leads sin contacto antes de revisar el historial completo." action={() => openEdit()} />
