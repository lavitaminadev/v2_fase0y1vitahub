...
export function VitaminaPulse({ compact = false }: { compact?: boolean }) {
  ...
  return <section className={`pulse-shell ${compact ? 'is-compact' : ''}`}>
    ...
    <div className="pulse-overview">
      ...
      <div className="commitment-card"><span>Compromisos abiertos</span>{!compact && <div><strong>{data.commitments.team}</strong><small>La Vitamina</small></div>}<div><strong>{data.commitments.client}</strong><small>{compact ? 'Tuyos' : 'Cliente'}</small></div></div>
    </div>
    ...
    <div className="pulse-lower">
      <div className="next-actions"><div className="pulse-subhead"><div><span className="page-eyebrow">PRÓXIMA MEJOR ACCIÓN</span><h3>Qué hacemos ahora</h3></div><small>Ordenado por impacto operativo</small></div>{visibleActions.map((action) => <Link to={action.route} className="pulse-action" key={action.id}><span className={`priority-dot is-${action.priority}`} /><div><strong>{action.title}</strong><small>{action.detail}</small></div><em>{action.owner === 'client' ? 'Cliente' : 'Equipo'} →</em></Link>)}{visibleActions.length === 0 && <p className="pulse-empty">{compact ? 'No tienes acciones pendientes por ahora.' : 'Sin acciones sugeridas con la evidencia disponible.'}</p>}</div>
